...
def collectDF():
    ...
    writePath = "popstar_results/distance_video_order1_compare_%s_%s.txt" % (inp1,inp2)
    ...
    for fname in dir_list:
        print(fname)
        dirname = fname
        readPath = "%s_analysis/distances_video_order1_%s.txt" % (inp1,dirname)
    ...
    for fname in dir_list:
        print(fname)
        dirname = fname
        readPath = "%s_analysis/distances_video_order1_%s.txt" % (inp2,dirname)
        df = pd.read_csv(readPath, sep = "\t")
        #print(df)
        for i in range(len(df)-1):
            distance = df.iloc[i,0]
...
def KLdiv():
    ...
    readPath = "popstar_results/distance_video_order1_compare_%s_%s.txt" % (inp1,inp2)
    df = pd.read_csv(readPath, sep = "\t")
    print(df)
...
def histPlot(p_value, ks_stat, kl_div):
    ...
    plt.legend()
    plt.savefig("popstar_results/compareDist_video_%s_%s.png" % (inp1,inp2))
    plt.show()
...
def main():
    ...
    print("\nstep distribution comparison on video is complete\n")   
