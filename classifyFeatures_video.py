...
def collectDFeb():
    print("collecting dataframe")
    writePath = "popstar_results/ternary_video_compare_%s.txt" % (folder_list)
...
def KruskalWallis():
    ...
    print(folder_list)
    readPath = "popstar_results/energy_video_compare_%s.txt" % (folder_list)
    df = pd.read_csv(readPath, sep = "\t")
    ...
    energy_p = round(p,3)
    readPath = "popstar_results/control_video_compare_%s.txt" % (folder_list)
    df = pd.read_csv(readPath, sep = "\t")
    ...
    control_p = round(p,3)
    readPath = "popstar_results/surprise_video_compare_%s.txt" % (folder_list)
    df = pd.read_csv(readPath, sep = "\t")
